...
<form action="{{ route('search.index') }}" method="get" class="needs-validation" novalidate>
    ...
    <div class="input-group mt-4 mb-4">
        <input type="search" class="form-control" placeholder="Tìm kiếm Bảng giá" name="search" id="validationCustom01" value="" required>
...
<div class="alert alert-danger alert-search" role="alert">
@if(count($articles) == 0)
<h3 class="text-center">Kết quả tìm kiếm cho từ khoá <b> {{ $search }} </b> là <b> 0 </b> kết quả</h3>
@endif
</div>
<div class="row">
<div class="col-sm-12 col-md-12 col-lg-9 col-xl-9 mb-4">
    <div class="alert alert-success alert-search" role="alert"> 
        <h3 class="text-center">Có thể bạn quan tâm ! Các bảng giá được xem nhiều nhất !</h3>
    </div>
    ...
    <div class="card shadow mb-4 card-custom">
        <div class="content">
            <div class="row">
                <div class="col-sm-5">
                    <a href="{{ $article->slug.'.html'}}" title="{{ $article->title }}"><img class="card-img-top img-fluid" src="/storage/uploads/{{ $article->img }}" alt="{{ $article->name }}" title="{{ $article->name }}"></a>
                </div>
                <div class="col-sm-7">
                ...
                <h3 class="card-title"><a href="{{ $article->slug.'.html'}}">{{ $article->name }}</a></h3>
