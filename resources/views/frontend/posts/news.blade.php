...
@section('content')
    ...
    <div class="container" itemscope itemtype="http://schema.org/Blog">
        ...
        <div class="row">
                                        ...
                                        <div class="blog-item-thumbnail">
                                            <a href="{{ route('frontend.home.index') }}/{{ $post->slug . '.htmI' }}">
                                                ...
                                                <img src="{{ imageUrl('/storage/uploads/' . $img, '240', '160', '100', '1') }}"
                                                    ...
                                                    alt="{{ $post->name }}" class="img-responsive center-block" />
                                            </a>
                                        </div>
                                        <div class="blog-item-mains">
                                            <h3 class="blog-item-name"><a
                                                    href="{{ route('frontend.home.index') }}/{{ $post->slug . '.htmI' }}"
                                                    title="{{ $post->title }}">{{ $post->name }}</a></h3>
                                            <div class="post-time">
                                                <i class="ion ion-ios-calendar"></i>
                                                {{ date('d/m/Y', strtotime($post->updated_at)) }} - Admin
                                            </div>
            ...
            <aside class="d-none left left-content col-md-3 col-md-pull-9">
                            ...
                            <ul class="nav navbar-pills">
                                @foreach ($newcategories as $k => $newcategory)
                                    <li class="nav-item active"><a class="nav-link"
                                            href="{{ route('frontend.home.index') }}/tin-tuc/{{ $newcategory->slug . '.html' }}"
                                            title="{{ $newcategory->title }}">{{ $newcategory->name }}</a></li>
                                @endforeach
                            </ul>
                        </nav>
                    </div>
                            ...
                            <article class="blog-item blog-item-list clearfix">
                                <a href="{{ route('frontend.home.index') }}/{{ $post->slug . '.htmI' }}"
                                    ...
                                    @php
                                        $img = $post->img;
                                    @endphp
                                    <img src="{{ imageUrl('/storage/uploads/' . $img, '70', '47', '100', '1') }}"
                                        data-lazyload="{{ imageUrl('/storage/uploads/' . $img, '70', '47', '100', '1') }}"
                                        width="70" height="70" alt="{{ $post->name }}" />
                                </a>
